...
static int xmusb350_qc_mode_select(struct xmusb350_charger *chip, u8 mode)
{
	...
	msleep(700);// need at least 120ms, then dp dm increase
	return ret;
}
...
static int xmusb350_hvdcp_enable(struct xmusb350_charger *chip, bool control)
{
	...
	if (control == ENABLE)
		msleep(700);// need 500ms, then QC20\QC30\QC35 detect
...
static void xmusb350_charger_type_det_work(struct work_struct *work)
{
	...
	if (xmusb_type == QC35_HVDCP_30) {
		for(i=0; i<5; i++)
		{
			...
			xmusb350_hvdcp_dpdm(chip, 0x8026); // 38 pulses
			...
			if(xmusb_type == QC35_HVDCP_30_27)
				break;
		}
...
static void xmusb350_charger_type_recheck(struct work_struct *work)
{
	...
	int ret = 0;
	union power_supply_propval pval = {0,};
	power_supply_get_property(chip->usb_psy, POWER_SUPPLY_PROP_VOLTAGE_NOW, &pval);

	if ((chip->chg_type == NONSTANDARD_CHARGER || chip->chg_type == CHARGER_UNKNOWN) && (pval.intval < 8000)) {
		chip->recheck_count++;
		//ret = xmusb350_write_byte_only(chip, XMUSB350_REG_RERUN_APSD);
		ret = xmusb350_write_byte(chip, XMUSB350_REG_RERUN_APSD, ENABLE);
		if (ret)
	...
	while(flag){
		...
		if((i==file_size)&&(flag == 1)){
			...
			for(i=0;i<file_size;i++){
				...
				xm35_err("i350 get fw data[%d]=0x%x  read_verify = 0x%x\n",i,*(file_w+i),read_data);

			    ...
			    enum bpf_access_type t, enum bpf_reg_type *reg_type)
{
	...
	if (env->prog->aux->verifier_ops->is_valid_access &&
	    env->prog->aux->verifier_ops->is_valid_access(off, size, t, env->prog, &info)) {
...
static int check_call(struct bpf_verifier_env *env, int func_id, int insn_idx)
{
	...
	if (env->prog->aux->verifier_ops->get_func_proto)
		fn = env->prog->aux->verifier_ops->get_func_proto(func_id, env->prog);
...
{
	const struct bpf_verifier_ops *ops = env->prog->aux->verifier_ops;
...
patch_call_imm:
		fn = prog->aux->verifier_ops->get_func_proto(insn->imm, prog);
